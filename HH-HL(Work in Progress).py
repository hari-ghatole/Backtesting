...
class FirstStrategy(bt.Strategy):
    ...
    def __init__(self):
        ...
        self.curent_hh = 99999
        self.curent_ll = -99999
    ...
    def next(self):
        ...
        self.current_date_string = "dd"
        self.current_time_string = "tt"
        ...
        if (self.datahigh[-6] < self.datahigh[-3] and self.datahigh[-5] < self.datahigh[-3] and self.datahigh[-4] < self.datahigh[-3]) and self.datahigh[-2] < self.datahigh[-3] and self.datahigh[-1] < self.datahigh[-3] and self.datahigh[0] < self.datahigh[-3]:
            self.log("H-H " + str(self.datahigh[-3]) + ", " +
                     self.current_date_string + " " + self.current_time_string)
            self.curent_hh = self.datahigh[-3]
        ...
        if self.datalow[-6] > self.datalow[-3] and self.datalow[-5] > self.datalow[-3] and self.datalow[-4] > self.datalow[-3] and self.datalow[-2] > self.datalow[-3] and self.datalow[-1] > self.datalow[-3] and self.datalow[0] > self.datalow[-3]:
            self.log("L-L " + str(self.datalow[-3]) + ", " +
                     self.current_date_string + " " + self.current_time_string)
            self.curent_ll = self.datalow[-3]

        if self.dataclose[0] > self.curent_hh:
            print("Long Signal : " +
                  str(self.dataclose[0]) + " HH : " + str(self.curent_hh))
            self.curent_hh = 9999999
        if self.dataclose[0] < self.curent_ll:
            print("Short Signal : " +
                  str(self.dataclose[0]) + " LL : " + str(self.curent_ll))
            self.curent_ll = -9999999
